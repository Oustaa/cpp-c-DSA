...
struct _Time {
  ...
  string stringify() {
    return to_string(min(hour, 11)) + ":" + (minute < 10 ? "0" : "") +
           to_string(min(minute, 59));
  }
};

void backTrack(vector<string> &result, vector<int> hours, vector<int> minuts,
               _Time t, int turnedOn, int current) {
  if (current >= turnedOn) {
